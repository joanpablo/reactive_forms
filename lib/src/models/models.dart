...
class FormControl<T> extends AbstractControl<T> {
  ...
  FormControl({
    T? value,
    this.toJson = _toJsonDefaultImpl,
    this.embedJsonToFormGroup = false,
    List<ValidatorFunction> validators = const [],
    List<AsyncValidatorFunction> asyncValidators = const [],
    int asyncValidatorsDebounceTime = 250,
    bool touched = false,
  ...
  @override
  dynamic get jsonValue => toJson(value);
...
class FormGroup extends AbstractControl<Map<String, Object?>>
  ...
  Map<String, dynamic> get jsonValue {
    ...
    _controls.forEach((key, control) {
      if (control.enabled || disabled) {
        if (control is FormControl && control.embedJsonToFormGroup) {
          final dynamic controlJson = control.jsonValue;

          if (controlJson is! Map<String, dynamic>?) {
            throw ArgumentError(
                'When embedJsonToFormGroup is true, the return value of jsonValue must be Map<String, dynamic>?');
          }
          if (controlJson != null) {
            assert(() {
              final intersection =
                  json.keys.toSet().intersection(controlJson.keys.toSet());
              if (intersection.isNotEmpty) {
                throw AssertionError(
                    'The field name($intersection) of control cannot be the same as the field name of formGroup.');
              }
              return true;
            }());

            json.addAll(controlJson);
          }
        } else {
          json[key] = control.jsonValue;
        }
      }
    });
    return json;
...
class FormArray<T> extends AbstractControl<List<T?>>
  ...
  @override
  List<dynamic> get jsonValue {
    return _controls
        .where((control) => control.enabled || disabled)
        .map<dynamic>((control) => control.jsonValue)
        .toList();
  }
